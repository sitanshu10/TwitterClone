...
let mutable totalNumTweets = int64 0
let mutable numTweets = int64 0
let mutable numZeroFollowing = int64 0
let mutable totalNumRetweets = int64 0
let mutable numRetweets = int64 0
...
let server (mailbox: Actor<_>) =
    ...
    let rec loop () =
        actor {
            ...
            | TweetAck -> numTweets <- numTweets + int64 1
            ...
            | RetweetAck -> numRetweets <- numRetweets + int64 1
        ...
        actor {
            ...
            | Initialize (numUsers) ->
                for i = 1 to numUsers do
                    ...
                    totalNumTweets <- totalNumTweets + int64 random
            ...
            | CalcNumZero ->
                for i = 1 to numUsers do
                    if clientToFollowing.[i].Count = 0 then
                        numZeroFollowing <-
                            numZeroFollowing
                            + int64 clientToFollowers.[i].Count
...
if numUsers < 10 then
    printfn "Please enter atleast 10 users for a proper simulation of Twitter."
else
